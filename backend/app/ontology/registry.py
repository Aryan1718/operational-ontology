...
class OntologyRegistry:
    ...
    enums_by_key: Mapping[str, tuple[str, ...]]
    ...
    def from_document(cls, document: Mapping[str, Any]) -> "OntologyRegistry":
        ...
        enums_by_key = MappingProxyType(
            {
                key: tuple(str(value) for value in values)
                for key, values in document.get("enums", {}).items()
            }
        )
        permissions = OntologyPermissionsDefinition.model_validate(
    ...
    def get_function(self, function_key: str) -> OntologyFunctionDefinition | None:
        """Return one function definition by API name."""
        return self.functions_by_key.get(function_key)

    def get_action_type(
        self, action_type: str
    ) -> OntologyActionTypeDefinition | None:
        """Return one action-type definition by API name."""
        return self.action_types_by_key.get(action_type)

    def get_role(self, role_key: str) -> OntologyRoleDefinition | None:
        """Return one role definition by API name."""
        return self.roles_by_key.get(role_key)
